...
func NewApp(ctx context.Context, config *configuration.Configuration) *App {
	...
	var err error
	app.driver, err = factory.Create(app.Context, config.Storage.Type(), storageParams)
	if err != nil {
	...
	startUploadPurger(app.Context, app.driver, dcontext.GetLogger(app.Context), purgeConfig)

	app.driver, err = applyStorageMiddleware(app.Context, app.driver, config.Middleware["storage"])
	if err != nil {
		panic(err)
	}
	...
	if redirectDisabled {
		dcontext.GetLogger(app.Context).Infof("backend redirection disabled")
	} else {
	...
	if cc, ok := config.Storage["cache"]; ok {
		...
		case "redis":
			...
			if _, ok := cc["blobdescriptorsize"]; ok {
				dcontext.GetLogger(app.Context).Warnf("blobdescriptorsize parameter is not supported with redis cache")
			}
			cacheProvider := rediscache.NewRedisBlobDescriptorCacheProvider(app.redis)
			localOptions := append(options, storage.BlobDescriptorCacheProvider(cacheProvider))
			app.registry, err = storage.NewRegistry(app.Context, app.driver, localOptions...)
			if err != nil {
				panic("could not create registry: " + err.Error())
			}
			dcontext.GetLogger(app.Context).Infof("using redis blob descriptor cache")
		case "inmemory":
			...
			dcontext.GetLogger(app.Context).Infof("using inmemory blob descriptor cache")
		default:
			if v != "" {
				dcontext.GetLogger(app.Context).Warnf("unknown cache type %q, caching disabled", config.Storage["cache"])
			}
		}
	}
	...
	app.registry, err = applyRegistryMiddleware(app.Context, app.registry, app.driver, config.Middleware["registry"])
	if err != nil {
		panic(err)
	}
	...
	if authType != "" && !strings.EqualFold(authType, "none") {
		...
		dcontext.GetLogger(app.Context).Debugf("configured %q access controller", authType)
	}
	...
	if config.Proxy.RemoteURL != "" {
		...
		app.isCache = true
		dcontext.GetLogger(app.Context).Info("Registry configured as a proxy cache to ", config.Proxy.RemoteURL)
	}
	var ok bool
	app.repoRemover, ok = app.registry.(distribution.RepositoryRemover)
	if !ok {
		dcontext.GetLogger(app.Context).Warnf("Registry does not implement RepositoryRemover. Will not be able to delete repos and tags")
	}
